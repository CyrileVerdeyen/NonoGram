...
public static int partSolve(int row[],int position, int fill, int offset){ //this takes a row and a restriction, plus an offset, and returns the partially solved array
    ...
    static boolean solveRecursiveSteps(int[][] board, int[] currentRestriction, int[][] rowRestrictions, int[][] columnRestrictions, int position, List<int[][]> boards){ //This is the recursive solver for the puzzle
        if (currentRestriction[0] >= board.length) { //all rows solved
            int[][] coppiedBoard = new int[board.length][board[0].length];
            for(int i = 0; i < board.length; i++){
                coppiedBoard[i] = board[i].clone();
            }
            boards.add(coppiedBoard);
            return true;
        }
          ...
          //This checks to see if a group that has not already been placed before can be placed in said position
            if(canPlace(board, currentRestriction, position, rowRestrictions, columnRestrictions, offset)){                
                ...
                if((currentRestriction[1] + 1) == rowRestrictions[currentRestriction[0]].length){
                    ...
                    if (solveRecursiveSteps(board, nextRestrictions, rowRestrictions, columnRestrictions, 0, boards)){
                        return true;
                    }
                } else {
                    ...
                    if (solveRecursiveSteps(board, nextRestrictions, rowRestrictions, columnRestrictions, position, boards)){
                        return true;
                    }
                }
    ...
    public static List<int[][]> solverStep(int board[][], int rowRestrictions[][], int columnRestrictions[][]){ //Calls the recursive function, if it can be completed it prints it, else it says it can not be printed.
        
        int [] currentRestriction = {0 ,0 };
        List <int[][]> boards = new ArrayList<int[][]>();
        if( solveRecursiveSteps(board, currentRestriction, rowRestrictions, columnRestrictions, 0, boards) ){
            //clPrint.print(board, rowRestrictions, columnRestrictions, true);
        } else {
        }
